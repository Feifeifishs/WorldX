...
- `contentLanguage`（字符串）：`"zh"`、`"en"` 或 `"ja"`。判断规则：如果用户描述使用日语、或故事背景明确处于日本/日本文化圈/日本学校实践场景，输出 `"ja"`；如果用户描述使用中文、或故事背景明确处于中国/中华文化圈（如宋朝、武侠、古代中国集市等），输出 `"zh"`；其他情况输出 `"en"`。此字段决定世界中所有生成内容（名字、对话、地名等）的语言
...
- 最多 8 个角色，最少 1 个
- 角色数量/姓名服从用户输入的硬约束：
  - 如果用户明确写了角色数量（如“3个人”“四名角色”“只要2个NPC”），`characters` 数组长度必须严格等于该数量；如果数量超过 8，返回 `feasible: false`
  - 如果用户明确列出了角色姓名（如“角色：张三、李四、王五”或“人物包括 Alice 和 Bob”），必须逐个原样保留这些姓名，`characters` 不得改名、漏掉或额外新增角色
  - 如果用户列出了身份/职业但没有姓名（如“铁匠、茶馆老板、云游僧人和孩子”），角色数量必须等于列出的身份数量，并且每个角色的 `role` 和 `appearance` 必须保留对应身份关键词
  - 如果用户同时给出“姓名 + 身份/职业”，必须保持一一对应，不要重新分配身份
  - 只有当用户没有明确指定人数、姓名或角色清单时，才可以根据场景需要自行决定 1–8 个角色
